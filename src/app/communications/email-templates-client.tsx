// /src/app/communications/email-templates-client.tsx
"use client";
...
export const EmailTemplatesClient = memo(() => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await getTemplates(userProfile, 'email');
      setTemplates(data);
    } catch (error) {
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Diseñe y guarde plantillas para sus campañas.</CardDescription>
          </div>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle>{selectedTemplate ? 'Editar Plantilla de Email' : 'Crear Nueva Plantilla de Email'}</DialogTitle>
          </DialogHeader>
          <TemplateForm type="email" template={selectedTemplate} onSuccess={handleFormSuccess} />
        </DialogContent>
      </Dialog>
...
EmailTemplatesClient.displayName = 'EmailTemplatesClient';
